...
def insert_academic_term(rows):
    ...
    new_terms = [
        AcademicTerm(term_code=term_code, suffix=suffix, start_date=start_date, end_date=end_date)
...
def insert_course_equivalent(rows):
    ...
    for row in rows:
        ...
        try:
            ...
            equivalent_course = Course.objects.get(department__code=equivalent_subject, catalog_number=equivalent_catalog_number)
        except Course.DoesNotExist:
            logging.warning(f"Course not found for Course ID: {primary_course_id} or Crosslisting: {equivalent_subject} {equivalent_catalog_number}")
            continue

        # Check if this equivalence already exists
        if not CourseEquivalent.objects.filter(primary_course=primary_course, equivalent_course=equivalent_course).exists():
            new_equivalent = CourseEquivalent(primary_course=primary_course, equivalent_course=equivalent_course)
...
def insert_class_year_enrollment(rows):
    ...
    for row in rows:
        ...
        try:
            section = Section.objects.get(course__course_id=course_id, term__term_code=term_code, class_number=class_number)
        except Section.DoesNotExist:
            logging.warning(f"Section not found for Course ID: {course_id}, Term Code: {term_code}, Class Section: {class_section}")
            continue

        if not ClassYearEnrollment.objects.filter(section=section, class_year=class_year).exists():
...
def insert_class_meeting(rows):
    ...
    for row in rows:
        ...
        # Fetch the section
        try:
            section = Section.objects.get(course__course_id=course_id, term__term_code=term_code, class_section=class_section)
        except Section.DoesNotExist:
            logging.warning(f"Section not found for Course ID: {course_id}, Term Code: {term_code}, Class Section: {class_section}")
            continue

        if not ClassMeeting.objects.filter(section=section, meeting_number=meeting_number).exists():
