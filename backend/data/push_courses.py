...
def process_department_rows(rows):
    ...
    departments = {
        (clean_string(row['Subject Code']), clean_string(row['Subject Name']))
        for row in rows
    }
    new_departments = [
...
def process_course_rows(rows):
    ...
    for row in rows:
        clean_row = {
            clean_string(key): clean_string(value) for key, value in row.items()
        }
        department, _ = Department.objects.get_or_create(code=clean_row['Subject Code'])
        defaults = {
            'catalog_number': clean_row['Catalog Number'],
