...
def generate_sentences(subjects, phrases, label, num_samples):
    return [
        (random.choice(phrases).format(random.choice(subjects)), label)
...
positive_samples = generate_sentences(subjects_list, positive_phrases, 1, num_samples)
negative_samples = generate_sentences(subjects_list, negative_phrases, 0, num_samples)
